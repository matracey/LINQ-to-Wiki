...
{
    private const string Tor1FileName = "TOR1.jpeg";
    private const string TestTitle1 = "Test";
    private const string TestTitle2 = "Ke smazání";
    ...
    {
        var token = wiki.tokens([tokenstype.block]).blocktoken;
    ...
    {
        var token = wiki.tokens([tokenstype.delete]).deletetoken;
    ...
    {
        var info = wiki.CreateTitlesSource("File:" + Tor1FileName)
            .Select(
                f =>
                new
                {
        ...
        var result = wiki.filerevert(Tor1FileName, archiveName, "I don't like this version of the image.", info.token);
    ...
    {
        var tokens = wiki.tokens([tokenstype.options]);
        wiki.options(tokens.optionstoken, change: ["skin=monobook"]);
    }
    ...
    private static void Protect(Wiki wiki)
    {
        ...
        var result = wiki.protect(["edit=autoconfirmed", "move=sysop"], token, "Test");
        Console.WriteLine(result);
    }

    private static void Purge(Wiki wiki)
    {
        var result = wiki.purge(titles: ["Test", "Test2", "Test3"], forcelinkupdate: true);
        Write(result);
    }
    ...
    {
        var tokens =
            wiki.CreateTitlesSource(TestTitle1).Select(
                p =>
                p.revisions()
                    ...
                    ToEnumerable())
                .ToEnumerable().Single();
        var result = wiki.rollback(TestTitle1, "127.0.0.1", tokens.First());
        Console.WriteLine(result);
    }
    ...
    private static void Undelete(Wiki wiki)
    {
        var deletedPage = wiki.Query.deletedrevs()
            ...
            .ToEnumerable()
            .Single(p => p.title == TestTitle2);

        var result = wiki.undelete(TestTitle2, deletedPage.token, "Just because");
        Console.WriteLine(result);
    }
    ...
    {
        var token = wiki.tokens([tokenstype.edit]).edittoken;
        var samplePhotoPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.CommonPictures), "Sample Pictures\\Chrysanthemum.jpg");
        using var stream = File.OpenRead(samplePhotoPath);
    ...
    private static void Watch(Wiki wiki)
    {
        ...
        var result = wiki.watch(token, titles: ["Test"], unwatch: false);
        Console.WriteLine(result);
    }
